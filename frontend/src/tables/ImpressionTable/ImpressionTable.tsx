...
import ImpressionHeader from "@/headers/ImpressionHeader/ImpressionHeader";
import ImpressionDetails from "@/details/ImpressionDetails/ImpressionDetails";
import { PAGE_SIZE } from "@/constants";
import Filters from "@/components/Filters/Filters";
import Controls from "@/components/Controls/Controls";
import Badges from "@/components/Badges/Badges";

interface ImpressionTableProps {
  className?: string;
}

export default ({ className }: ImpressionTableProps) => {
  const [selectedProgram, setSelectedProgram] = useState(null);
  const [pageNum, setPageNum] = useState(1);

  const { data, error, isLoading } = useQuery({
    queryKey: ["impression", selectedProgram, pageNum],
    queryFn: () => {
      return impressionService.searchImpression({
  ...
  return (
    <div className={`${className}`}>
      ...
      <Controls
        ...
        openFilters={open}
        shareUrl="/impression/add"
        shareText="Share Impression"
      />
