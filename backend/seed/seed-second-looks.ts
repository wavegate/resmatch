...
const jsonFilePath = join(process.cwd(), "seedData", "secondLookData.json");
...
async function createFameShameEntries() {
  for (const entry of fameShameData) {
    ...
    const { nrmpProgramCode, ...fameShameDetailsData } = entry;
    ...
    try {
      ...
      const fameShameEntry = await prisma.secondLook.create({
        data: {
          userId: userId,
