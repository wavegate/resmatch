...
const jsonFilePath = join(process.cwd(), "seedData", "fameShameData.json");
...
async function createFameShameEntries() {
  for (const entry of fameShameData) {
    ...
    const { comments, nrmpProgramCode, ...fameShameDetailsData } = entry;
    ...
    try {
      ...
      const fameShameEntry = await prisma.fameShame.create({
        data: {
          userId: userId,
          programId: program.id, // Use the found program's ID
          ...fameShameDetailsData,
